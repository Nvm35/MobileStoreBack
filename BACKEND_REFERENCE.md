...
# Склады
GET    /api/admin/warehouses                   # Список всех складов
...
- Номера заказов теперь формата `ORD-YYMMDD-XXXXXX`, например `ORD-241117-3F2A7C`, и появляются в ответе сразу после создания. Их можно безопасно использовать в UI, ссылках и в админке.
...
# Добавить в корзину по slug
POST /api/cart
{
  "product": "chehol-apple-iphone-15-pro",
  "quantity": 1
}

# Создать заказ с понятными идентификаторами
POST /api/orders
{
  "items": [
    {
      "product_slug": "chehol-apple-iphone-15-pro",
      "product_variant_sku": "APPLE-CASE-IP15P-BLACK",
      "quantity": 2
    }
  ],
